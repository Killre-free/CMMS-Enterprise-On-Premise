...
function CreateTemplateForm({ onDone }: { onDone: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : t("createTemplateFailed"));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      ...
      >
        {submitting ? t("creating") : t("createTemplate")}
      </button>
    </form>
  );
}
        ...
        .map((f) => (
          <div key={f.id}>
            ...
            {f.type === "Boolean" ? (
              ...
              <input disabled placeholder={t("computedAutomatically")} className={inputClass} />
            ) : (
              <input
...
export default function CheckSheetsPage() {
  const tr = useTranslations("CheckSheets");
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        >
          <Plus size={16} /> {tr("newTemplate")}
        </button>
      </div>

      <div className="rounded-lg border border-border bg-background p-4">
        <h2 className="mb-2 text-sm font-medium">{tr("templates")}</h2>
        {templatesLoading && <p className="text-sm text-muted-foreground">{tr("loading")}</p>}
        {templates?.data.length === 0 && (
          <p className="text-sm text-muted-foreground">{tr("noTemplatesYet")}</p>
        )}
        <ul className="flex flex-col gap-2">
          {templates?.data.map((t) => (
            <li key={t.id} className="flex items-center justify-between border-b border-border pb-2 text-sm last:border-0">
              <span>
                {t.name} <span className="text-xs text-muted-foreground">v{t.version} · {t.fields.length} {tr("fieldsCount")}</span>
              </span>
      ...
      <div className="rounded-lg border border-border bg-background p-4">
        <h2 className="mb-2 text-sm font-medium">{tr("recentSubmissions")}</h2>
        {submissionsLoading && <p className="text-sm text-muted-foreground">{tr("loading")}</p>}
        {submissions?.data.length === 0 && (
          <p className="text-sm text-muted-foreground">{tr("noSubmissionsYet")}</p>
        )}
        <ul className="flex flex-col gap-2">
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title={tr("newTemplate")}>
